...
const SingleItem = ({ item, isNegotiated = false, onRemoveNegotiated }: any) => {
  const [quantity, setQuantity] = useState(item.quantity);

  const dispatch = useDispatch<AppDispatch>();

  const handleRemoveFromCart = async () => {
    if (isNegotiated && onRemoveNegotiated) {
      onRemoveNegotiated(item.id);
      return;
    }
  ...
  return (
    <div className={`flex items-center border-t border-gray-3 py-5 px-7.5 relative ${
      isNegotiated
        ? 'bg-gradient-to-r from-yellow-50/50 via-white to-transparent'
        : ''
    }`}>
      {isNegotiated && (
        <>
          <div className="absolute top-2 right-7.5 bg-yellow-400 text-dark text-xs px-3 py-1 rounded font-bold flex items-center gap-1">
            🎁 Oferta Especial -{item.negotiatedDiscount}%
          </div>
          <div className="absolute top-2 left-7.5 text-yellow-400 text-sm animate-pulse">
            ✨
          </div>
        </>
      )}
        ...
        <div className="flex items-center justify-between gap-5">
          <div className="w-full flex items-center gap-5.5">
            <div className={`flex items-center justify-center rounded-[5px] max-w-[80px] w-full h-17.5 ${
              isNegotiated ? 'bg-yellow-50' : 'bg-gray-2'
            }`}>
              <Image width={200} height={200} src={item.imgs?.thumbnails[0]} alt="product" />
            </div>

            <div>
              <h3 className={`ease-out duration-200 hover:text-blue ${
                isNegotiated ? 'text-yellow-700 font-semibold' : 'text-dark'
              }`}>
                <a href="#"> {item.title} </a>
              </h3>
            </div>
      ...
      <div className="min-w-[180px]">
        <p className={isNegotiated ? 'text-yellow-700 font-bold' : 'text-dark'}>
      ...
      <div className="min-w-[275px]">
        {isNegotiated ? (
          <div className="w-max flex items-center justify-center bg-yellow-50 rounded-md px-6 py-3">
            <span className="text-yellow-700 font-bold">Cantidad: {quantity}</span>
          </div>
        ) : (
      ...
      <div className="min-w-[200px]">
        <p className="text-dark">S/ {(item.discountedPrice * quantity).toFixed(2)}</p>
      </div>

      <div className="min-w-[50px] flex justify-end">
        {isNegotiated ? (
          <div className="flex items-center justify-center max-w-[38px] w-full h-9.5 bg-yellow-100 rounded-lg">
